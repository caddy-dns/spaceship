...
func TestUnmarshalCaddyfile_Block(t *testing.T) {
	...
	if p.APIKey != "key123" || p.APISecret != "secret456" { t.Fatalf("credentials mismatch") }
	if p.PageSize != 250 { t.Fatalf("expected pagesize 250 got %d", p.PageSize) }
	if p.HTTPClient == nil || p.HTTPClient.Timeout != 15 * time.Second { t.Fatalf("expected timeout 15s got %#v", p.HTTPClient) }
	if p.BaseURL != "https://example.test/api" { t.Fatalf("unexpected base url: %s", p.BaseURL) }
}
...
func TestUnmarshalCaddyfile_DuplicateKey(t *testing.T) {
	...
	toks := tokenize("test.caddy", input)
	d := caddyfile.NewDispenser(toks)
	if err := p.UnmarshalCaddyfile(d); err == nil { t.Fatalf("expected error for duplicate api_key") }
}
...
func TestProvision_EnvFallbackAndPlaceholders(t *testing.T) {
	...
	if err := p.Provision(ctx); err != nil { t.Fatalf("Provision failed: %v", err) }
	if p.APIKey != "envkey" || p.APISecret != "envsecret" { t.Fatalf("placeholder expansion failed: %s / %s", p.APIKey, p.APISecret) }
}
